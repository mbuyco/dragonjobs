...
Define the automated CI/CD workflow that keeps the job board fresh by running the ingest pipeline on a schedule, exporting jobs to static JSON, and conditionally deploying only when data has changed.
